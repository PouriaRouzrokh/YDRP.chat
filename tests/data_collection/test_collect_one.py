...
def test_collect_one():
    ...
    try:
        ...
        print(
            f"Raw Timestamped Markdown files found ({len(raw_markdown_files_found)}): {raw_markdown_files_found}"
        )
        assert (
            len(raw_markdown_files_found) > 0
        ), "No raw timestamped markdown file was created."
        ...
        expected_dir_pattern = re.compile(
            r".+_\d{20}$"
        )  # Ends with _<20-digit-timestamp>
        ...
        print(
            f"Processed Policy output directories found ({len(policy_dirs_found)}): {policy_dirs_found}"
        )
        assert (
            len(policy_dirs_found) > 0
        ), "No processed policy output directory (<title>_<timestamp>) was created."

        # Check contents of the first directory found
        policy_output_dir_name = policy_dirs_found[0]
        policy_output_dir = os.path.join(
            config.PATHS.SCRAPED_POLICIES_DIR, policy_output_dir_name
        )
        ...
        assert os.path.exists(
            content_md_path
        ), f"content.md not found in {policy_output_dir}"
        print(f"Found: {content_md_path}")
        assert os.path.exists(
            content_txt_path
        ), f"content.txt not found in {policy_output_dir}"
        ...
        assert (
            txt_size <= md_size
        ), "content.txt is larger than content.md (filtering failed?)"
        ...
        images_in_policy_dir = [
            f
            for f in os.listdir(policy_output_dir)
            if f.lower().endswith((".png", ".jpg", ".jpeg", ".gif"))
        ]
        print(
            f"Images found in policy dir ({len(images_in_policy_dir)}): {images_in_policy_dir}"
        )
