...
from ydrpolicy.backend.database.models import (
    Base, Policy, PolicyChunk, Image, create_search_vector_trigger
)
    ...
    try:
        ...
        try:
            ...
            return True # Indicate success (created or existed)
             ...
             logger.error(f"Database '{db_name}' does not exist (caught InvalidCatalogNameError). Cannot connect directly.")
...
async def create_extension(engine: AsyncEngine, extension_name: str) -> None:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(f"CREATE EXTENSION IF NOT EXISTS {extension_name} SCHEMA public") # Specify schema
            )
...
async def process_new_policy_folder(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"  Failed to generate embeddings for policy '{policy_title}': {emb_err}. Skipping chunk creation.")
...
async def populate_database_from_scraped_policies():
    ...
    if os.path.exists(csv_path):
        try:
            ...
            if 'extraction_reasoning' in policy_df.columns:
                ...
                logger.info(f"Loaded {len(timestamp_to_description)} timestamp mappings and {len(url_to_description)} URL mappings for policy descriptions")
            else:
    ...
    async with get_async_session() as session:
        try:
            ...
            for folder_name in os.listdir(scraped_policies_dir):
                ...
                logger.debug(f"Matched folder name: '{folder_name}' → title='{policy_title}', timestamp={scrape_timestamp}")
                ...
                if policy_title in existing_policies:
                    ...
                    if existing_metadata and 'scrape_timestamp' in existing_metadata:
                        ...
                        if existing_timestamp >= scrape_timestamp:
                            logger.debug(f"Skipping older or same version of policy: '{policy_title}', existing={existing_timestamp}, new={scrape_timestamp}")
                            skipped_count += 1
                            should_process = False
                        else:
                            logger.info(f"Found newer version of policy: '{policy_title}', existing={existing_timestamp}, new={scrape_timestamp}")
    ...
    logger.info(f"Database population finished. Processed {processed_count} new policies. Skipped {skipped_count} folders.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--drop",
        action="store_true",
        help="Drop the database (USE WITH CAUTION!)."
    )
    parser.add_argument(
        "--db_url",
        help="Optional database URL to override config."
    )
    parser.add_argument(
        "--no-populate",
        action="store_true",
